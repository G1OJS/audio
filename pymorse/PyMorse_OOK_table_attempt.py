...
def define_figure(nf):
    ...
    tbl = axs[1].table(cellText = tbl_text, bbox = [0, p.y0-0.11, 1, p.y1+0.12], edges = 'open', colWidths = [0.1,0.4,0.5])    
    return fig, axs, tbl_text
...
def run(input_device_keywords, freq_range, df, hop_ms, display_decimate, n_decoders, show_processing):
    ...
    fig, axs, tbl_text = define_figure(spectrum.nf)
    ...
    animation_artists_list = spec_plot, *keylines
    ...
    def animation_callback(frame):
        ...
        return [spec_plot, *axs[0].lines, *axs[1].texts]

    ani = FuncAnimation(plt.gcf(), animation_callback, interval = 30, frames = 100000,  blit = False)
    plt.show()
